...
/*
PROGRAMMA da usare DOPO aver eseguito il comando hadd (vedi agenda)
...
void ADHD_BKGrec(TString fname){
  ...
  const double time = 200; // time to distinguish between prompt and after "capture" energy release


  // ---- CREATING NEW BRANCHES ---- //

    // hod3
  double Hod3_prompt_E=0, Hod3_Acap_E=0;
  ...
  TBranch *Br_Hod3_Acap_E = ADHD->Branch(branchName.Data(),&Hod3_Acap_E,branchName+"/D");
    ...
    // hod2
  double Hod2_prompt_E=0, Hod2_Acap_E=0;
  ...
  TBranch *Br_Hod2_Acap_E = ADHD->Branch(branchName.Data(),&Hod2_Acap_E,branchName+"/D");
    ...
    //hit number
  int HodHit_prompt=0, HodHit_Acap_new=0;
  ...
  TBranch *Br_beta = ADHD->Branch(branchName.Data(),&beta,branchName+"/D");
    ...
    // HeCal
  double HeCal_prompt_E=0, HeCal_Acap_E=0;
  ...
  for (Int_t i = 0; i < nev; i++) {
    ...
    // initialization
    HeCal_prompt_E = 0, HeCal_Acap_E = 0;
    Hod3_prompt_E = 0, Hod3_Acap_E = 0;
    Hod2_prompt_E = 0, Hod2_Acap_E = 0;
    HodHit_prompt=0, HodHit_Acap_new=0;
    beta=0;
    ...
    for(UInt_t j=0; j<Hod3N; j++){
      ...
      if(Hod3E->at(j)!=0 && Hod3T->at(j)<time){
    ...
    for(UInt_t j=0; j<Hod2N; j++){
      ...
      if(Hod2E->at(j)!=0 && Hod2T->at(j)<time) {
  ...
  ADHD->Write("",TObject::kOverwrite); // save only the new version of the tree
  File->Write();
  delete File;
  return;
}
