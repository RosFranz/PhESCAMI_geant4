...
void ADHD_BKGrec(TString fname){
  ...
  // ---- CREATING NEW BRANCHES ---- //
    ...
    // hod3
  double Hod3_rec_E=0;
  TString branchName = "Hod3_rec_E";
  TBranch *Br_Hod3_rec_E = ADHD->Branch(branchName.Data(),&Hod3_rec_E,branchName+"/D");

    // hod2
  double Hod2_rec_E=0;
  branchName = "Hod2_rec_E";
  TBranch *Br_Hod2_rec_E = ADHD->Branch(branchName.Data(),&Hod2_rec_E,branchName+"/D");
  ...
  double hitScint[NScint];
    ...
    //hit number
  int HodHit_rec=0;
  branchName = "HodHit_rec";
  TBranch *Br_HodHit_rec = ADHD->Branch(branchName.Data(),&HodHit_rec,branchName+"/I");
  ...
  double dist;
    ...
    // HeCal
  double HeCal_rec_E=0;
  branchName= "HeCal_rec_E";
  TBranch *Br_HeCal_rec_E = ADHD->Branch(branchName.Data(),&HeCal_rec_E,branchName+"/D");
  ...
  for (Int_t i = 0; i < nev; i++) {
    ...
    for(UInt_t j=0; j<HGasN; j++){
        ...
        if(HGasE->at(j)>0) hitE[copy] = hitE[copy] + HGasE->at(j);
    }
    for(int tank=0; tank<NTanks; tank++) {
      if(hitE[tank]>HeCal_rec_E) HeCal_rec_E = hitE[tank];
      hitE[tank] = 0;
    }
    Br_HeCal_rec_E->Fill();
    ...
    for(int hod=0; hod<NScint; hod++){
      if(hitScint[hod]>0 && hitScint[hod]>Hod3_rec_E) Hod3_rec_E = hitScint[hod];
      hitScint[hod] = 0;
    }
      //prompt energy Hod2
    ...
    for(int hod=0; hod<NScint; hod++){
        if(hitScint[hod]>0 && hitScint[hod]>Hod2_rec_E) Hod2_rec_E = hitScint[hod];
        hitScint[hod] = 0;
    }
    ...
    printf("\rProgress percentage %d / 100\n",int((i*1.0)/nev*100.));
    if(i==nev-1) printf("\n");
  }
