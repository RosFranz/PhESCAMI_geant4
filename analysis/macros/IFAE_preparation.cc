...
void IFAE_preparation(){
    ...
    TString names[NFiles] = {"../../sources/ROOT_files/Dbar_10e6_gps_SM1rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM1rec_TESTdir__50ns.root",
                             "../../sources/ROOT_files/Dbar_10e6_gps_SM2rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM2rec_TESTdir__50ns.root",
                             "../../sources/ROOT_files/Dbar_10e6_gps_SM3rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM3rec_TESTdir__50ns.root",
                             "../../sources/ROOT_files/Dbar_10e6_gps_SM4rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM4rec_TESTdir__50ns.root",
                             "../../sources/ROOT_files/Dbar_10e6_gps_SM5rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM5rec_TESTdir__50ns.root",
                             "../../sources/ROOT_files/Dbar_10e6_gps_SM6rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM6rec_TESTdir__50ns.root",
                             "../../sources/ROOT_files/Dbar_10e6_gps_SM7rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM7rec_TESTdir__50ns.root",
                             "../../sources/ROOT_files/Dbar_10e6_gps_SM8rec_TESTdir__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM8rec_TESTdir__50ns.root"};

    

    // TString names[NFiles] = {"../../sources/ROOT_files/Dbar_10e6_gps_SMrec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SMrec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM1rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM1rec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM2rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM2rec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM3rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM3rec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM4rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM4rec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM5rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM5rec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM6rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM6rec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM7rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM7rec__50ns.root",
    //                          "../../sources/ROOT_files/Dbar_10e6_gps_SM8rec__50ns.root", "../../sources/ROOT_files/pbar_10e6_gps_SM8rec__50ns.root"};
    ...
    const double MIP_min = 0.5, MIP_max = 10.5;
    ...
    TH2D * h2_TOTAL_EHecal_VS_Hod3 = new TH2D("h2_TOTAL_EHecal_VS_Hod3", "HeCal prompt energy vs HOD3 prompt energy; HeCal Prompt Energy [MeV]; dE/dx [MIPs]", nBins_HeCalP, HeCal_minP, HeCal_maxP, nBins_MIP, MIP_min, MIP_max);
    ...
    double TOTHeCalLate=0, TOTHod2Slablate=0, TOTHod3SlabLate=0, TOTHeCalTankLate=0, TOTHeCalTankPrompt=0;
                ...
                (NHod3Prompt+NHod2Prompt) < th_HodHitTot ){
                    ...
                    if( (Hod3_MaxELate > th_Hod_low) && 
                        ...
                        for(int tank=0; tank<HeCal_E_tankMyPrompt->size(); tank++) {
                            if(HeCal_E_tankMyPrompt->at(tank)>0.) {
                                TOTHeCalPrompt += HeCal_E_tankMyPrompt->at(tank);
                                TOTHeCalTankPrompt++;
                            }
                            if(HeCal_E_tankLate->at(tank) > 0.)  TOTHeCalLate += HeCal_E_tankLate->at(tank);
                            if(HeCal_E_tankLate->at(tank) > 2) TOTHeCalTankLate++;
                        }
                        ...
                        if(i%2 == 0){
                            if(TOTHeCalTankPrompt==1){
                                h2_Eprompt_VS_Beta[Dbar]->Fill(TOTHeCalPrompt, beta);
                                h2_EHecal_VS_Hod2[Dbar]->Fill(TOTHeCalPrompt, TOTHod2Prompt);
                                h2_EHecal_VS_Hod3[Dbar]->Fill(TOTHeCalPrompt, TOTHod3Prompt/(PathLenght*MIP_norm));
                            }
                            
                            h2_EHecalL_VS_Hod2SlabL[Dbar]->Fill(TOTHeCalLate, NHod2Late);
                            h2_EHecalL_VS_Hod3SlabL[Dbar]->Fill(TOTHeCalLate, NHod3Late);
                            h2_EHecalL_VS_HodSlabL[Dbar]->Fill(TOTHeCalLate, NHod2Late+NHod3Late);
                            h2_EHecalL_VS_HeCalTankL[Dbar]->Fill(TOTHeCalLate, TOTHeCalTankLate);

                            h2_TOTAL_EHecalL_VS_TotalHit_part[Dbar]->Fill(TOTHeCalLate, NHod2Late+NHod3Late+TOTHeCalTankLate);
                            ...
                            if(TOTHeCalTankPrompt==1){
                                h2_Eprompt_VS_Beta[Pbar]->Fill(TOTHeCalPrompt, beta, weight);
                                h2_EHecal_VS_Hod2[Pbar]->Fill(TOTHeCalPrompt, TOTHod2Prompt, weight);
                                h2_EHecal_VS_Hod3[Pbar]->Fill(TOTHeCalPrompt, TOTHod3Prompt/(PathLenght*MIP_norm), weight);
                            }

                            h2_EHecalL_VS_Hod2SlabL[Pbar]->Fill(TOTHeCalLate, NHod2Late, weight);
                            h2_EHecalL_VS_Hod3SlabL[Pbar]->Fill(TOTHeCalLate, NHod3Late, weight);
                            h2_EHecalL_VS_HodSlabL[Pbar]->Fill(TOTHeCalLate, NHod2Late+NHod3Late, weight);
                            h2_EHecalL_VS_HeCalTankL[Pbar]->Fill(TOTHeCalLate, TOTHeCalTankLate, weight);

                            h2_TOTAL_EHecalL_VS_TotalHit_part[Pbar]->Fill(TOTHeCalLate, NHod2Late+NHod3Late+TOTHeCalTankLate, weight);
    ...
    TFile *f_out = new TFile ("plots_florence_testdir.root", "RECREATE");
    f_out->cd();
    for(int i_hist=0; i_hist<NParticles; i_hist++) {
        ...
        for(int i=1; i<=h2_EHecalL_VS_Hod2SlabL[i_hist]->GetNbinsX(); i++){
            for(int j=1; j<=h2_EHecalL_VS_Hod2SlabL[i_hist]->GetNbinsY(); j++){
                if(h2_EHecalL_VS_Hod2SlabL[i_hist]->GetBinContent(i,j)==0) h2_EHecalL_VS_Hod2SlabL[i_hist]->SetBinContent(i,j,-1);
            }
        }
        h2_EHecalL_VS_Hod2SlabL[i_hist]->Write();

        for(int i=1; i<=h2_EHecalL_VS_Hod3SlabL[i_hist]->GetNbinsX(); i++){
            for(int j=1; j<=h2_EHecalL_VS_Hod3SlabL[i_hist]->GetNbinsY(); j++){
                if(h2_EHecalL_VS_Hod3SlabL[i_hist]->GetBinContent(i,j)==0) h2_EHecalL_VS_Hod3SlabL[i_hist]->SetBinContent(i,j,-1);
            }
        }
        h2_EHecalL_VS_Hod3SlabL[i_hist]->Write();

        for(int i=1; i<=h2_EHecalL_VS_HodSlabL[i_hist]->GetNbinsX(); i++){
            for(int j=1; j<=h2_EHecalL_VS_HodSlabL[i_hist]->GetNbinsY(); j++){
                if(h2_EHecalL_VS_HodSlabL[i_hist]->GetBinContent(i,j)==0) h2_EHecalL_VS_HodSlabL[i_hist]->SetBinContent(i,j,-1);
            }
        }
        h2_EHecalL_VS_HodSlabL[i_hist]->Write();

        for(int i=1; i<=h2_EHecalL_VS_HeCalTankL[i_hist]->GetNbinsX(); i++){
            for(int j=1; j<=h2_EHecalL_VS_HeCalTankL[i_hist]->GetNbinsY(); j++){
                if(h2_EHecalL_VS_HeCalTankL[i_hist]->GetBinContent(i,j)==0) h2_EHecalL_VS_HeCalTankL[i_hist]->SetBinContent(i,j,-1);
            }
        }
        h2_EHecalL_VS_HeCalTankL[i_hist]->Write();

        for(int i=1; i<=h2_TOTAL_EHecalL_VS_TotalHit_part[i_hist]->GetNbinsX(); i++){
            for(int j=1; j<=h2_TOTAL_EHecalL_VS_TotalHit_part[i_hist]->GetNbinsY(); j++){
                if(h2_TOTAL_EHecalL_VS_TotalHit_part[i_hist]->GetBinContent(i,j)==0) h2_TOTAL_EHecalL_VS_TotalHit_part[i_hist]->SetBinContent(i,j,-1);
            }
        }
    ...
    for(int i=1; i<=h2_TOTAL_EHecalL_VS_Hod2SlabL->GetNbinsX(); i++){
        for(int j=1; j<=h2_TOTAL_EHecalL_VS_Hod2SlabL->GetNbinsY(); j++){
            if(h2_TOTAL_EHecalL_VS_Hod2SlabL->GetBinContent(i,j)==0) h2_TOTAL_EHecalL_VS_Hod2SlabL->SetBinContent(i,j,-1);
        }
    }
    h2_TOTAL_EHecalL_VS_Hod2SlabL->Write();

    for(int i=1; i<=h2_TOTAL_EHecalL_VS_Hod3SlabL->GetNbinsX(); i++){
        for(int j=1; j<=h2_TOTAL_EHecalL_VS_Hod3SlabL->GetNbinsY(); j++){
            if(h2_TOTAL_EHecalL_VS_Hod3SlabL->GetBinContent(i,j)==0) h2_TOTAL_EHecalL_VS_Hod3SlabL->SetBinContent(i,j,-1);
        }
    }
    h2_TOTAL_EHecalL_VS_Hod3SlabL->Write();

    for(int i=1; i<=h2_TOTAL_EHecalL_VS_HodSlabL->GetNbinsX(); i++){
        for(int j=1; j<=h2_TOTAL_EHecalL_VS_HodSlabL->GetNbinsY(); j++){
            if(h2_TOTAL_EHecalL_VS_HodSlabL->GetBinContent(i,j)==0) h2_TOTAL_EHecalL_VS_HodSlabL->SetBinContent(i,j,-1);
        }
    }
    h2_TOTAL_EHecalL_VS_HodSlabL->Write();

    for(int i=1; i<=h2_TOTAL_EHecalL_VS_HeCalTankL->GetNbinsX(); i++){
        for(int j=1; j<=h2_TOTAL_EHecalL_VS_HeCalTankL->GetNbinsY(); j++){
            if(h2_TOTAL_EHecalL_VS_HeCalTankL->GetBinContent(i,j)==0) h2_TOTAL_EHecalL_VS_HeCalTankL->SetBinContent(i,j,-1);
        }
    }
    h2_TOTAL_EHecalL_VS_HeCalTankL->Write();

    for(int i=1; i<=h2_TOTAL_EHecalL_VS_TotalHit->GetNbinsX(); i++){
        for(int j=1; j<=h2_TOTAL_EHecalL_VS_TotalHit->GetNbinsY(); j++){
            if(h2_TOTAL_EHecalL_VS_TotalHit->GetBinContent(i,j)==0) h2_TOTAL_EHecalL_VS_TotalHit->SetBinContent(i,j,-1);
        }
    }
